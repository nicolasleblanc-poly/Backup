...
def callback_output_structure(iteration, of_list, rho):
    ...
    simulation = viz_sim(epsr, source,'tmp4/epsr_%03d.png' % iteration)
...
simm = viz_sim(epsr,source,"ReChi4_grey_omega.pdf")
...
print(objective(rho_optimum),flush=True)
print(np.max(np.max(epsr)),np.min(np.min(epsr)),flush=True)
simm = viz_sim(epsr,source,"ReChi4_bin_omega.pdf")
