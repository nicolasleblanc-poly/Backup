...
import time,sys
sys.path.append("/home/nic-molesky_lab/Desktop/Nic's code/Extracted-power-optimization/photonic-dual-bounds") #load package directory into system path so the script can access the package
...
from dualbound.Lagrangian.spatialProjopt_dualgradHess_fakeS_singlematrix_numpy import get_spatialProj_dualgrad_fakeS_singlematrix 
def get_TE_dipole_oneside_LDOS_Msparse_bound(chi, wvlgth, orient, design_x, design_y, dist, pml_sep, pml_thick, gprx, gpry, NProjx, NProjy, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, alg='LBFGS'):
    """
    ...
    G=get_Yee_TE_GreenFcn(wvlgth, Mx, My, Npmlx, Npmly, dx, dy, Qabs=1e4)
    print("G", G,"\n")
    ...
    O_quad = sp.csc_matrix(Gddinv.shape, dtype='complex')

    include = np.ones(2*len(UPlist), dtype='bool')

    print('starting dual optimization', flush=True)
    
    optLags, optgrad, dualval, objval, gradZTT, gradZTS_S = get_Msparse_bound(S1, O_lin, O_quad, GinvdagPdaglist, UPlist, include, dualconst=vacPrad, opttol=opttol, fakeSratio=fakeSratio, reductFactor=reductFactor, iter_period=iter_period, alg=alg)
     
    # Code to get T, dualval and gradient to check if my code (Nic) is good
    fSlist = []
    Lags = np.zeros(int(np.sqrt(len(G))), dtype='complex')
    grad = np.zeros(len(Lags), dtype='complex')
    print("gradZTT", type(gradZTT),"\n")
    print("gradZTS_S", type(gradZTS_S),"\n")

    optimization = get_spatialProj_dualgrad_fakeS_singlematrix(Lags, grad, O_lin, O_quad, gradZTS_S, gradZTT, fSlist)
...
wvlgth = 1
orient = np.array([1,1])
design_x = np.double(0.1)
design_y = np.double(0.1)
dist = 20
pml_sep = np.double(0.5*wvlgth)
pml_thick = 0.5*wvlgth
gprx = 20
gpry = 20
NProjx = 1
NProjy = 1
