...
def DipoleOverSlab_setup(dx,dy,addx,liste):
# Questions:
...
# Important constant there
    ...
    return M,ei,Pv
# First constraint
def C1(T,i,ei_tr, chi_invdag, Gdag):
    # Left term
    P=Pv[i]
    PT=np.matmul(P,T)
...
# Second constraint
def C2(T,i,ei_tr, chi_invdag, Gdag):
    # Left term
    P=Pv[i]
    PT=np.matmul(P,T)
    # E_tc = np.matrix.conjugate(np.transpose(ei))
...
def Am(x, chi_invdag, Gdag): # the A matrix
        A=0.0+0.0j
...
def th_solve(x,A,b):
    ...
    return T,A
def cg_solve(x,A,b):
    T=spla.cg(A,b,tol=1e-14)[0]
    return T,A
def bicg_solve(x,A,b):
    T=spla.bicg(A,b,tol=1e-14)[0]
    return T
def bicgstab_solve(x,A,b):
    T=spla.bicgstab(A,b,tol=1e-14)[0]
    return T
tsolvers = {
    "th": th_solve,
    "cg": cg_solve,
...
def tsolverpick(name,x,A,b):
    return tsolvers[name](x,A,b)
def Dual(x,g,fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, get_grad):
    ...
    A=Am(x, chi_invdag, Gdag)
    b=bv(x,ei)
    solve=tsolverpick(tsolver,x,A,b)
    T=solve[0]
    # A=solve[1]
    g=np.ones(len(x),dtype='complex')
    for i in range(len(x)):
        if i % 2 ==0:
            g[i] = C1(T,math.floor(i/2),ei_tr, chi_invdag, Gdag) 
        else:
            g[i] = C2(T,math.floor(i/2),ei_tr, chi_invdag, Gdag)
    D=0.0+0.0j
    if len(fSlist)==0:
    ...
    if get_grad == True:
        ...
        return D # D.real
def mineigfunc(x, chi_invdag, Gdag):
    A=Am(x, chi_invdag, Gdag)
    # print(A)
    w_val,v=la.eig(A)
    # w: eigen values
...
def validityfunc(x, chi_invdag, Gdag):
    val=mineigfunc(x, chi_invdag, Gdag)
    min_eval=val[0]
    min_evec=val[1]
    if min_eval>0:
        return 1
        ...
        return -1
def BFGS_fakeS_with_restart(initdof, dgfunc, validityfunc, mineigfunc,tsolver, ei, ei_tr, chi_invdag, Gdag, gradConverge=False, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6):
    dofnum = len(initdof)
    ...
    prev_dualval = np.inf
    dof = initdof
    justfunc = lambda d: dgfunc(d, np.array([]), fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, get_grad=False)
    olddualval = np.inf
    reductCount = 0
    while True: #gradual reduction of modified source amplitude, outer loop
        ...
        val = dgfunc(dof, grad, fSlist, tsolver,ei,  ei_tr, chi_invdag, Gdag, get_grad=True)  
        dualval = val[0]
        grad = val[1]
#             print("grad",grad,"\n")
...
#                 print('starting alpha', alpha_start)
            while validityfunc(dof+alpha*pdir, chi_invdag, Gdag)<=0: #move back into feasibility region
                alpha *= c_reduct
            alpha_feas = alpha
...
#                         print('encountered feasibility wall, adding a fake source term')
                    ...
                    eig_fct_eval = mineigfunc(singular_dof, chi_invdag, Gdag) #find the subspace of ZTT closest to being singular to target with fake source
...
#                         print("min_evec type",type(mineigv),"\n")
                    ...
                    fakeSval = dgfunc(dof, np.array([]), [mineigv], tsolver, ei, ei_tr, chi_invdag, Gdag, get_grad=False)
...
#                 print('stepsize alphaopt is', alphaopt, '\n')
            ...
            tmp_val = dgfunc(dof+delta, tmp_grad, fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, get_grad=True)
            tmp_dual = tmp_val[0]
            tmp_grad = tmp_val[1]
...
def item(x0):
    ...
    '''
    if tsolver=="cg":
        val = Dual_cg(x0,g,fSlist, get_grad)
    elif tsolver=="bicg":
        val = Dual_bicg(x0,g,fSlist, get_grad)
    elif tsolver=="bicgstab":
        val = Dual_bicgstab(x0,g,fSlist, get_grad)
    dualval = val[0]
    grad = val[1]
    '''
    Ak=0
    ...
    for k in range(0,len(x0)-1):
        ...
        yk=(1-bk)*zk+bk*xk
        xk=yk-(1/L)*grad*yk
        zk=(1-q*dk)*zk+q*dk*yk-(dk/L)*g(yk)
    '''
    return xk
lsolvers = {
    "bfgs": BFGS_fakeS_with_restart,
    "item": item}
def lsolverpick(name,x,tsolver,ei, ei_tr,chi_invdag,Gdag):
    return lsolvers[name](x,Dual,validityfunc, mineigfunc,tsolver,ei, ei_tr,chi_invdag,Gdag)
...
def extracted_power_optmization(M,ei,x0,tsolver,lsolver,Pv):
    ...
    start = timer()
    x_th=lsolverpick(lsolver,x0,tsolver,ei,ei_tr,chi_invdag,Gdag)
    gradient=x_th[-3]
    dual=x_th[-2]
    objective=x_th[-1]
    stop = timer()
...
setup=DipoleOverSlab_setup(dx,dy,addx,liste)
print("setup",setup)
M=setup[0]
ei=setup[1]
Pv=setup[2]
...
dy=2
addx=1
liste=[[1,1,1,1,1,1,1,1],[10,12,14,16,2,4,6,8]]
setup=DipoleOverSlab_setup(dx,dy,addx,liste)
M=setup[0]
ei=setup[1]
Pv=setup[2]
...
'''
'''
# Test to see if the code in julia works like the code in python. 
wvlgth = 1.0
Qabs = 1e4
omega = (2*np.pi/wvlgth) * (1+1j/2/Qabs)
get_pml_x(omega, 2, 2, 1, 0.5)
'''
'''
# Test to see if the code in julia works like the code in python. 
wvlgth = 1.0
...
G = get_Yee_TE_GreenFcn(wvlgth, Mx, My, Npml, Npml, dL, dL, Qabs=Qabs)
print("G",G)
'''
