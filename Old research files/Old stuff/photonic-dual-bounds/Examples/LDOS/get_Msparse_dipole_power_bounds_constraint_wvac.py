...
def get_TM_dipole_Prad_Msparse_bound(chi, wvlgth, design_x, design_y, vacuum_x, vacuum_y, dist, pml_sep, pml_thick, gpr, NProjx, NProjy, NProjvx, NProjvy, opttol=1e-2, fakeSratio=1e-2, iter_period=200):
    ...
    halfx = Mx//2
    halfy = My//2
    for i in range(NProjvx):
        for j in range(NProjvy):
            Upro = np.zeros((Mx*My,Mx*My))
            for ii in range(halfx-vacuumx//2+int(np.round(i*(vacuumx//2+int(max([vacuumx//2,np.ceil(vacuumx//2)])))/NProjvx)),halfx-vacuumx//2+int(np.round((i+1)*(vacuumx//2+int(max([vacuumx//2,np.ceil(vacuumx//2)])))/NProjvx))):
                for kk in range(halfy-vacuumy//2+int(np.round(j*(vacuumy//2+int(max([vacuumy//2,np.ceil(vacuumy//2)])))/NProjvy)),halfy-vacuumy//2+int(np.round((j+1)*(vacuumy//2+int(max([vacuumy//2,np.ceil(vacuumy/2)])))/NProjvy))):
                    Upro[ii+Mx*kk,ii+Mx*kk] = 1.0
    ...
    include = [True] * (2*(len(proj_ulco_list)+NProjvx*NProjvy)) #include all constraints

    print("include ",(2*(len(proj_ulco_list)+NProjvx*NProjvy)),flush=True)
