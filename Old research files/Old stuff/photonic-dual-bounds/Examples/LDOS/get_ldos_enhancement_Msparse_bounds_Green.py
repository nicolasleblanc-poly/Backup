...
def get_Msparse_bound(chi, Si, O_lin, O_quad, Ginv, Plist, include, dualconst=0.0, initLags=None, getViolation=False, opttol=1e-2, fakeSratio=1e-2, iter_period=1000):
    ...
    if getViolation:
        ...
        return optLags, optgrad, dualval, objval, np.real(violation)
...
def get_Msparse_1designrect_normal_absorption_violation_splitting(chi, wvlgth, design_x, design_y, vacuum_x, vacuum_y, emitter_x, emitter_y, pml_sep, pml_thick, gpr, initLags=None, fakeSratio=1e-2, iter_period=1000, name='test'):
...
#    for qq in range(10):    
        try:
            ...
            new_subLags, subPlist = split_sparseP_by_violation(optLags[2:], subPlist, violation)
