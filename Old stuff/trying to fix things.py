...
def get_Yee_TE_GreenFcn(wvlgth, Gx,Gy, Npmlx, Npmly, dx,dy, addx, Qabs=np.inf):
    """
    ...
    dipx = int(np.ceil(addx/2)) # index of the dipole in the x 
    half_lenG=int(len(G)/2) # gives the last index in the x
    # dipx=int(i/2)-1 # index of dipole in the x
    dipy=half_lenG+dipx # index of the dipole in the y
    dim=2*Gx*Gy
    a=np.zeros((dim,1),dtype='complex')
    b=np.zeros((dim,1),dtype='complex')
    a[:,0]=G[:,dipx] # the column of G for the source in the x
    b[:,0]=G[:,dipy] # the column of G for the source in the y
    coeff = (-Z*1j)/(k0)
    alpha = 1/math.sqrt(2)
    beta = 1/math.sqrt(2)
    ei_tot = coeff*(alpha*a+beta*b) 
    print("ei_tot", ei_tot, "\n")
    return G, ei_tot
...
# aka the dimension of the user's problem might be way bigger than the values below
    ...
    Gcall = get_Yee_TE_GreenFcn(wvlgth, Mx, My, Npml, Npml, dL, dL, addx, Qabs=Qabs)
    Gbig=Gcall[0]
    ei_tot=Gcall[1]
    # print("Gbig",Gbig,"\n")
...
# Important constant there
    ...
    ei = u
    # print("ei", ei, "/n")
    ei_p = np.ones((len(ei),1), dtype='complex')
    ei_p[0] = 0.00385335+0.00385024j
    ei_p[1] = 0.00504074+0.00274553j
    ei_p[2] = 0.00242696+0.00483549j
    ei_p[3] = 0.00389187+0.00416524j
    ei_p[4] = 0.2797116 +0.64901378j
    ei_p[5] = 0.28076192+0.64761974j
    ei_p[6] = 0.06525505+0.70330879j
    ei_p[7] = 0.06668143+0.70232355j
    for i in range(len(ei)):
        print("ei[i]/ei_p[i]", np.linalg.norm(ei[i])/ np.linalg.norm(ei_p[i]), "\n")
...
def Dual(x,g,fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, get_grad, e_vac):
    ...
    if len(fSlist) == 0:
        ei_T=np.dot(ei_tr,T)
        obj = 0.5**(k0/Z)*np.imag(ei_T) + e_vac  # + vaccuum contribution
        ...
        ei_T=np.dot(ei_tr,T)
        obj = 0.5*(k0/Z)*np.imag(ei_T) + e_vac + fAf # + vacuum contribution
        # ((k0/(2*Z))*
        D = obj
        for i in range(len(x)):
            D += np.real(x[i]*g[i])
    ...
    if get_grad == True:
        return np.real(D), np.real(g), np.real(obj), T, A, b # D.real
    elif get_grad == False:
        return np.real(D), np.real(obj), T, A, b # D.real
...
def extracted_power_optmization(M,ei,x0,tsolver,lsolver,Pv, e_vac):
    ...
    print("Done solving")
    mult = np.real(x_th[0])
    gradient = np.real(x_th[1])
    dual = np.real(x_th[2])[0][0]
    objective = np.real(x_th[3])[0][0]
    T = x_th[4]
    A = x_th[5]
    b = x_th[6]
...
# T_p = np.ones((len(T),1), dtype='complex')
# # T_p[0] = -0.01235934-0.01725768j 
# # T_p[1] = 0.02197872+0.06604921j
# # T_p[2] =  -0.00922071-0.01748019j
# # T_p[3] = 0.01760838+0.09001024j
# # T_p[4] = -0.12988986-0.31501118j
# # T_p[5] = -0.12211787-0.29661536j
# # T_p[6] = -0.09454901-0.23882399j
# # T_p[7] = -0.09768764-0.23860149j

# for i in range(len(T)):
#     print("T[i]/T_p[i]", np.linalg.norm(T[i])/ np.linalg.norm(T_p[i]), "\n")
...
# Pv=setup[2]
# e_vac=setup[3]
# x0_2=np.ones(2*len(liste),dtype='complex')
# x0_2[0]=1.5
# x0_2[1]=1.5
# tsolver="th"
# lsolver="bfgs"
