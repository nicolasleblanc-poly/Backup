...
def get_spatialProj_dualgrad_fakeS_singlematrix(Lags, grad, O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=0.0, include=None):
    if include is None:
        include = [True]*len(Lags)
...
def get_spatialProj_dualgradHess_fakeS_singlematrix(Lags, grad, Hess, O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=0.0, include=None):
    if include is None:
        include = [True]*len(Lags)
...
def get_inc_spatialProj_2source_dualgradHess_fakeS_singlematrix(incLags, incgrad, incHess, include, O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=0.0, get_grad=True, get_Hess=True):
    Lagnum = len(include)
    Lags = np.zeros(Lagnum)
    Lags[include] = incLags[:]
    ...
    if get_Hess:
        ...
        dualval = get_spatialProj_dualgradHess_fakeS_singlematrix(Lags,grad,Hess, O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=dualconst, include=include)
        incgrad[:] = get_incgrad_from_grad(grad, include) #[:] since we are modifying in place
        incHess[:,:] = get_incHess_from_Hess(Hess, include)
    elif get_grad:
        grad = np.zeros(Lagnum)
        dualval = get_spatialProj_dualgrad_fakeS_singlematrix(Lags,grad, O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=dualconst, include=include)
        incgrad[:] = get_incgrad_from_grad(grad, include)
    else:
        dualval = get_spatialProj_dualgrad_fakeS_singlematrix(Lags,np.array([]), O, UPlist, S, ZTS_Sfunc, gradZTS_Sfunc, fSlist, dualconst=dualconst, include=include)
    return dualval
