...
function Dual(x,g,fSlist, tsolver, get_grad)
    ...
    if length(fSlist)==0
        ...
        # print("fSlist", fSlist, "\n")
        # f=ones(ComplexF64, length(fSlist[1]), 1)
        # if length(fSlist)==1
        #     f = fSlist[1]
        # else
...
function mineigfunc(x)
    ...
    # print("v", v, "\n")
    min_evec=v[:,1]
    print("min_eval calc ", min_eval, "\n")
    # print("min_evec calc ", min_evec, "\n")
    # print("w_val", w_val, "\n")
    # w: eigen values
    # v: eigen vectors
    # w=real(w_val)
...
function BFGS_fakeS_with_restart(initdof, dgfunc, validityfunc, mineigfunc, tsolver, gradConverge=false, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6)
    ...
    function justfunc(d, fSlist)
        return dgfunc(d, Array([]), fSlist, tsolver, false)
    end
...
function item(x0, Dual, validityfunc, tsolver)
    mu=10e-8
    L=10e6
    q=mu/L
    Ak=0
...
# # item function call
# # sln_item = item(x0, Dual, validityfunc, "th")
# # print("dof: ", sln_item[1], "\n")
# # print("grad: ", sln_item[2], "\n")
# # print("dualval: ", sln_item[3], "\n")
