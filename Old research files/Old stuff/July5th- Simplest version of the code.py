...
def get_Yee_TE_GreenFcn(wvlgth, Gx,Gy, Npmlx, Npmly, dx,dy, Qabs=np.inf):
    ...
    # print("x_Ex.flatten()",x_Ex.flatten().shape ,"\n")
    ...
    ei[len(orient_Exfield):,0] = orient_Eyfield
...
# aka the dimension of the user's problem might be way bigger than the values below
    ...
    ei=np.zeros((dim,1),dtype='complex')
    ei[:position] = ei_tot[i:half_lenGbig]
    ei[position:] = ei_tot[j:lenGbig]
...
# Important constant there
    ...
    for element in liste:
        P=np.diag(element)
        Pv.append(P)
    # Let's get e_vac (electric field in vacuum)
    dim_vac = 2*Mx*My
    pos_vac = int(len(Gbig)/2)
    a=np.zeros((dim_vac,1),dtype='complex')
    b=np.zeros((dim_vac,1),dtype='complex')
    a[:,0]=Gbig[:,dipx] # the first index is the rows
    # a[pos_vac:,0]=Gbig[:,dipx] # the second index is the columns
    b[:,0]=Gbig[:,dipy] 
    # b[pos_vac:,0]=Gbig[:,dipy]
    alpha_vac=1/(math.sqrt(2))
    beta_vac=1/(math.sqrt(2))
    # coeff=(Z*1j)/(k0)
    # ei=alpha*u+beta*v # this gives ei for a dipole at 45 degrees
    ei_vac=(alpha_vac*a+beta_vac*b) #coeff*
    ji = np.zeros((len(ei_vac),1),dtype='complex')
    ji[dipx] = 1
    ji[dipy] = 1
    e_vac = np.vdot(ji,ei_vac)
...
def Dual(x,g,fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, get_grad, e_vac):
    ...
    g[0] = C1(T,0,ei_tr, chi_invdag, Gdag, Pv) 
    D=0.0+0.0j
    if len(fSlist)==0:
        ei_T=np.matmul(ei_tr,T)
        D = ((-1/2)*np.real(ei_T*k0*1.0j/(Z)))+ e_vac
        D+=x[0]*g[0]
    else:
        ...
        ei_T=np.matmul(ei_tr,T)
        D = ((-1/2)*np.real(ei_T*k0*1.0j/(Z)))+e_vac+fAf + x[0]*g[0]
    if get_grad == True:
        return D, g, T # D.real
    elif get_grad == False:
        return D, T # D.real
...
def extracted_power_optmization(M,ei,x0,tsolver,lsolver,Pv, e_vac):
    ...
    start = timer()
    x_th=BFGS_fakeS_with_restart(x0,Dual,validityfunc, mineigfunc,tsolver,ei, ei_tr,chi_invdag,Gdag, Pv, e_vac)
    print("Done solving")
    gradient=x_th[-4]
    dual=x_th[-3]
    objective=x_th[-2]
...
Pv=setup[2]
e_vac=setup[3]
x0=np.ones(len(liste),dtype='complex')
tsolver="th"
lsolver="bfgs"
...
Pv=setup[2]
e_vac=setup[3]
x0_2=np.ones(len(liste),dtype='complex')
x0_2[0]=1.05
tsolver="th"
lsolver="bfgs"
