...
def get_TM_dipole_Prad_Msparse_bound(chi, wvlgth, design_x, design_y, vacuum_x, vacuum_y, emitter_x, emitter_y, dist, pml_sep, pml_thick, gpr, NProjx, NProjy, opttol=1e-2, fakeSratio=1e-2, iter_period=1000):
    ...
    design_mask[Npml+Npmlsep+Dx+halfx-vacuumx//2:Npml+Npmlsep+Dx+halfx+int(max([vacuumx//2,np.ceil(vacuumx/2)])), Npml+Npmlsep+halfy-vacuumy//2:Npml+Npmlsep+halfy+int(max([vacuumy//2,np.ceil(vacuumy/2)]))] = False
    ...
    for i in range(len(proj_brco_list)):
        ...
        if halfx-vacuumx//2 <= proj_ulco[0] and halfx+int(max([vacuumx//2,np.ceil(vacuumx/2)])) >= proj_brco[0] and halfy-vacuumy//2 <= proj_ulco[1] and halfy+int(max([vacuumy//2,np.ceil(vacuumy/2)])) >= proj_brco[1]:
            len_ulco_list -= 1
            continue
        for k in range(halfy+int(max([vacuumy//2,np.ceil(vacuumy/2)])),halfy-vacuumy//2,-1):
            for ii in range(halfx+int(max([vacuumx//2,np.ceil(vacuumx/2)])),halfx-vacuumx//2,-1):
                Proj_mat = np.delete(Proj_mat,ii+Mx*k)
    ...
    try:
        ff = open("Lags_file_2.txt","r")
        lines = ff.readlines()
        for ii in range(len(Lags)):
            Lags[ii] = float(lines[ii])
        ff.close()
    except:
        pass
