...
def item(x0, Dual, validityfunc, mineigfunc,tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, gradConverge=False, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6):
    mu=10e-8
    L=10e6
    q=mu/L
    Ak=0
    ...
    val_xk = Dual(xk, grad, fSlist, tsolver,ei,  ei_tr, chi_invdag, Gdag, Pv, get_grad=True)  
    dualval_xk = val[0]
    grad_xk = val[1]
    ...
    # prev_dualval = np.inf
    dof = x0
    justfunc = lambda d: Dual(d, np.array([]), fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, get_grad=False)
    olddualval = np.inf
    reductCount = 0
    it=0
    while True: #gradual reduction of modified source amplitude, outer loop
        fSlist = [] #remove old fake sources and restart with each outer iteration
        alpha_last = 1.0 #reset step size
        Hinv = np.eye(x0,dtype='complex') #reset Hinv
        val = Dual(dof, grad, fSlist, tsolver,ei,  ei_tr, chi_invdag, Gdag, Pv, get_grad=True)  
        dualval = val[0]
        grad = val[1]
    #             print("grad",grad,"\n")
                # This gets the value of the dual function and the dual's gradient
    ...
    #                         print("min_evec type",type(mineigv),"\n")
                            # fakeSval = dgfunc(dof, np.matrix([]), matrix, get_grad=False)
                    fakeSval = Dual(dof, np.array([]), [mineigv], tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, get_grad=False)
                            # This only gets us the value of the dual function
                    epsS = np.sqrt(fakeSratio*np.abs(dualval/fakeSval))
    #                         print('epsS', epsS, '\n')
                            #fSlist.append(np.matmul(epsS,mineigv))
    ...
    #                 print('stepsize alphaopt is', alphaopt, '\n')
            ...
            tmp_val = Dual(dof+delta, tmp_grad, fSlist, tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, get_grad=True)
            tmp_dual = tmp_val[0]
            tmp_grad = tmp_val[1]
            ...
            p_dot_tmp_grad = pdir @ tmp_grad
            if added_fakeS:
                Hinv = np.eye(x0,dtype='complex') #the objective has been modified; restart Hinv from identity
            elif p_dot_tmp_grad > c_W*p_dot_grad: #satisfy Wolfe condition, update Hinv
    #                     print('updating Hinv')
                # BFGS update
                gamma = tmp_grad - grad
...
def Cpp_Execution(enter):
    ...
    os.write(temp, bytes("7 12 25\n", "utf-8"));
    os.close(temp)
    # temp is for writing and data is for reading
    # storing output as a byte string
    s = subprocess.check_output("g++ test.cpp -o out1;./out1", stdin = data, shell = True)
...
def nomad(x0, dgfunc, validityfunc, mineigfunc,tsolver, ei, ei_tr, chi_invdag, Gdag, Pv, gradConverge=False, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6):
    ...
    grad = np.zeros(len(x0))
    fSlist = [] 
    eval = Dual(x0, grad, fSlist, tsolver,ei,  ei_tr, chi_invdag, Gdag, Pv, get_grad=False)
    obj = eval[1]
    C1_store = eval[2]
    C2_store = eval[3]
    liste = [obj]
    for i in range(len(C1_store)):
        liste.append(C1_store[i])
        liste.append(C2_store[i])
