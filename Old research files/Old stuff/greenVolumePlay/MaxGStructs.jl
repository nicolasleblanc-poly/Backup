...
"""
function MaxGAssemblyOpts(scale::NTuple{3,Float64})

	return MaxGAssemblyOpts(1.0 + im * 0.0,
		min(max(floor(Int, 22 + 3 * log10(maximum(scale))), 1), 1), 
		min(max(floor(Int, 22 + 3 * log10(maximum(scale))), 1), 4),
		min(max(floor(Int, 22 + 3 * log10(maximum(scale))), 2), 16),
		min(max(round(Int, 2 * ^(2, -log10(minimum(scale)) * 5 / 2 - 3 / 2)), 
		4), 16), min(max(round(Int, 1 * ^(2, -log10(minimum(scale)) * 5 / 2 - 
		3 / 2)), 2), 8))
end
