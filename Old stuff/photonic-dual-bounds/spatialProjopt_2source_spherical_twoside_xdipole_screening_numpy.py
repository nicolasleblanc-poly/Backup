...
from eqconstraint_xdipole_spherical_domain import get_real_RgNM_l_coeffs_for_xdipole_field, get_real_RgNM_l_coeffs_for_plusz_xdipole_field


def get_sphere_twoside_xdipole_screening_datalists_numpy(k, dist, Ro, Rd_Plist, chi,
    ...
    while True:
        ...
        #####calculate the coefficients for Si, including both +z and -z xdipole waves####
        np.seterr(under='warn')
        #the +z xdipole
        xdp_coeffs = np.array(get_real_RgNM_l_coeffs_for_plusz_xdipole_field(l,k,Rall_list[-1]+dist, wig_1llp1_000, wig_1llm1_000, wig_1llp1_1m10, wig_1llm1_1m10))
        
        #construct the plusz_xdipole M source vector
        xdp_M_Sivec = np.zeros(UPlistlist[2*l-2][0].shape[0], dtype=np.complex)
        for i in range(len(subbasis_indlistlist[2*l-2])-1): #-1 because last index of subbasis_indlist is total number of basis vectors
            xdp_M_Sivec[subbasis_indlistlist[2*l-2][i]] = np.complex(xdp_coeffs[1]*Rgnormlistlist[2*l-2][i])/np.sqrt(2) #factor of 1/sqrt(2) comes from normalizing the real azimuthal vector spherical harmonics


        #construct the plusz_xdipole N source vector
        xdp_N_Sivec = np.zeros(UPlistlist[2*l-1][0].shape[0], dtype=np.complex)
        for i in range(len(subbasis_indlistlist[2*l-1])-1): #-1 because last index of subbasis_indlist is total number of basis vectors
            xdp_N_Sivec[subbasis_indlistlist[2*l-1][i]] = np.complex(xdp_coeffs[2]*Rgnormlistlist[2*l-1][i])/np.sqrt(2)

        #the coeffs of the source in terms of the non-normalized regular waves with real azimuthal angle dependence
        xdm_coeffs = np.array(get_real_RgNM_l_coeffs_for_xdipole_field(l,k,Rall_list[-1]+dist, wig_1llp1_000, wig_1llm1_000, wig_1llp1_1m10, wig_1llm1_1m10))
        ...
        xdm_M_Sivec = np.zeros(UPlistlist[2*l-2][0].shape[0], dtype=np.complex)
        for i in range(len(subbasis_indlistlist[2*l-2])-1): #-1 because last index of subbasis_indlist is total number of basis vectors
            xdm_M_Sivec[subbasis_indlistlist[2*l-2][i]] = np.complex(xdm_coeffs[1]*Rgnormlistlist[2*l-2][i])/np.sqrt(2) #factor of 1/sqrt(2) comes from normalizing the real azimuthal vector spherical harmonics


        #construct the xdipole N source vector
        xdm_N_Sivec = np.zeros(UPlistlist[2*l-1][0].shape[0], dtype=np.complex)
        for i in range(len(subbasis_indlistlist[2*l-1])-1): #-1 because last index of subbasis_indlist is total number of basis vectors
            xdm_N_Sivec[subbasis_indlistlist[2*l-1][i]] = np.complex(xdm_coeffs[2]*Rgnormlistlist[2*l-1][i])/np.sqrt(2)


        Silist.append(np.concatenate((xdp_M_Sivec, xdm_M_Sivec)))
        Silist.append(np.concatenate((xdp_N_Sivec, xdm_N_Sivec)))
        ...
        xdp_sosqr = np.real(np.vdot(xdp_M_Sivec,xdp_M_Sivec)+np.vdot(xdp_N_Sivec,xdp_N_Sivec))
        xdp_sumnorm += xdp_sosqr
        print('plusz xdipole sumnorm', xdp_sumnorm)
        
        xdm_sosqr = np.real(np.vdot(xdm_M_Sivec,xdm_M_Sivec)+np.vdot(xdm_N_Sivec,xdm_N_Sivec))
        xdm_sumnorm += xdm_sosqr
        print('minusz xdipole sumnorm,', xdm_sumnorm)
        if l>2 and xdp_sosqr<xdp_sumnorm*normtol and xdm_sosqr<xdm_sumnorm*normtol:
            #break when both xdipole norm summation tolerances have been met
            break
...
def get_sphere_twoside_xdipole_equalnorm_screening_numpy(k, dist, Ro, Rd_Plist, chi, 
                                                 Silist, Salist, Olist, Gmat_od_list, Po_list,
                                                 Plistlist, UPlistlist, Rgnormlistlist, subbasis_indlistlist, 
                                                 wig_1llp1_000,wig_1llm1_000, wig_1llp1_1m10,wig_1llm1_1m10, 
    ...
    #np.seterr(all='raise')
    mp.dps=mpdps
    klim_update = get_sphere_twoside_xdipole_screening_datalists_numpy(k, dist, Ro, Rd_Plist, chi,
                                                 Silist, Salist, Olist, Gmat_od_list, Po_list,
                                                 Plistlist, UPlistlist, Rgnormlistlist, subbasis_indlistlist, 
                                                 wig_1llp1_000,wig_1llm1_000, wig_1llp1_1m10,wig_1llm1_1m10, 
...
def sweep_norm_remainder_spherical_twoside_xdipole_equalnorm_screening_varydist(k, distlist, Ro, Rd_Plist, chi, 
    ...
    for i in range(len(distlist)):
        ...
        try:
            if (not feedforward) or (Lags is None):
                ...
                Lags, totdual, totobj, totnormsqrratio, dual1, dual2, Si1normsqr, Si2normsqr, normsqrratio1, normsqrratio2, klim = get_sphere_twoside_xdipole_equalnorm_screening_numpy(k, dist, Ro, Rd_Plist, chi, 
