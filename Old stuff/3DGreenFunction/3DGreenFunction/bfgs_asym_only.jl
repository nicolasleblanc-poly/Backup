module bfgs_asym_only
export BFGS_fakeS_with_restart_x
...
function BFGS_fakeS_with_restart_x(initdof,dgfunc,P,ei,e_vac,g_xx, g_xy, g_xz, g_xx_dag, g_xy_dag, g_xz_dag,cellsA,gradConverge=false, opttol=1e-2, fakeSratio=1e-2, reductFactor=0.1, iter_period=20, min_iter=6)
    # print("Entered BFGS")
    ...
    function justfunc(d,fSlist)
        return dgfunc(d,Array([]),P,ei,e_vac,g_xx, g_xy, g_xz, g_xx_dag, g_xy_dag, g_xz_dag,cellsA,fSlist,false)
    end
    ...
    while true #gradual reduction of modified source amplitude, outer loop
        ...
        Hinv = I #reset Hinv
        val = dgfunc(dof,grad,P,ei,e_vac,g_xx, g_xy, g_xz, g_xx_dag, g_xy_dag, g_xz_dag,cellsA,fSlist,true)  
        print("Done")
        dualval = val[1]
        grad = val[2]
        obj = val[3]
...
#                         print("min_evec type",type(mineigv),"\n")
                    ...
                    fakeS_eval = dgfunc(dof,Array([]),P,ei,e_vac,g_xx, g_xy, g_xz, g_xx_dag, g_xy_dag, g_xz_dag,cellsA,[mineigv],false)
...
#                         print('epsS', epsS, '\n')
            ...
            tmp_val = dgfunc(dof+delta,tmp_grad,P,ei,e_vac,g_xx, g_xy, g_xz, g_xx_dag, g_xy_dag, g_xz_dag,cellsA,fSlist, true)
