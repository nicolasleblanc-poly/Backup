...
from dualbound.Rect.mask_iterative_splitting import split_sparseP_by_violation, plot_sparseP_distribution, get_max_violation_sparseP, get_max_violation_sparseP_2
...
def get_Msparse_1designrect_normal_absorption_violation_splitting(chi, wvlgth, design_x, design_y, vacuum_x, vacuum_y, emitter_x, emitter_y, pml_sep, pml_thick, gpr, initLags=None, fakeSratio=1e-2, tol_lindep=1e-6, tol_dualchange=1e-3, iter_period=1000, name='test',len_sp_name='lsp.txt',sp_root_name='sparse_p',lags_name='Lags.txt'):
    ...
    #violation = np.zeros(subPlist[0].shape[0])
...
#    for qq in range(10):    
    ...
    #lags_file.close()
    #len_sp_file = open(len_sp_name+str(np.imag(chi)),"w")
    #len_sp_file.write(str(len_sp))
    #len_sp_file.close()
    #for ll in range(len_sp):
    #    sp.save_npz(sp_root_name+str(ll)+str(np.imag(chi))+'.npz',subPlist[ll])
    #lags_file = open(lags_name+str(np.imag(chi)),"w")
    #np.savetxt(lags_file,Lags)
    #lags_file.close()
